...
contract ChainPongEscrow {
    ...
    mapping(bytes32 => bool)      public matchIdUsed;           // explicit CSPRNG matchId uniqueness guard
    ...
    ///         matchId must be a CSPRNG bytes32 generated by the backend.
    function createMatch(
        bytes32 matchId,
        uint256 deadline,
        bytes calldata permit
    ) external payable whenNotPaused nonReentrant {
        require(!matchIdUsed[matchId], "Match ID already used");
        require(matches[matchId].state == MatchState.Empty, "Match ID exists");
        require(validStakes[msg.value], "Invalid stake amount");

        // Mark matchId as permanently used (even if later cancelled/disputed)
